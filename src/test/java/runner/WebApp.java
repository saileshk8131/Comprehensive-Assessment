...
public class WebApp extends BaseTest {
	...
	static pageobject.AirportPage airportpage;
	static OutstationPage outstationpage;
	static pageobject.LocalPage localpage;
	static pageobject.SitemapPage sitepage;
	...
	public static void SitemapPage() {
			...
			test = extent.createTest("SitemapPage");// df
			test.log(Status.PASS, "TestCase Execution Started");// ff
			loggerUtil = new Logs();
			log = loggerUtil.createLog("SitemapPage");
			pageManager = new PageObjectManager();
			sitepage = pageManager.getSitemapPage();
			sitepage.clickonsitemap();
			sitepage.Switchtab(1);
			sitepage.clickoncabtypes();
			sitepage.Switchtab(2);
			sitepage.clickonTataindica();
			sitepage.Switchtab(3);
			sitepage.verifyheading();
		}
